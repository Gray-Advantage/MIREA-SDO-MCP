...
``submit`` делает тот самый финальный POST. Стадии «черновик ответа» здесь нет:
сохранение сразу переводит работу в «Отправлено для оценивания». Отыграть назад
обычно можно — Moodle показывает «Редактировать ответ» и «Удалить ответ», пока
не вышел срок и работа не оценена, — но рассчитывать на это нельзя, поэтому
инструмент требует явного подтверждения.
"""
...
async def stage_files(
    ...
    return {
        ...
        "submitted": False,
        "next_step": (
            "Файлы лежат в черновичной области и НЕ сданы. В интерфейсе задания их "
            "пока не видно — черновик привязан к этому открытию формы. Чтобы сдать, вызови "
...
async def submit(
    ...
    if not confirm:
        return {
            ...
            "message": (
                "Сохранение сразу переводит работу в «Отправлено для оценивания» — "
                "стадии черновика здесь нет. Обычно ответ можно потом заменить или "
                "снять, пока не вышел срок и работа не оценена, но полагаться на это "
                "не стоит. Спроси пользователя и повтори вызов с confirm=true."
            ),
        }
